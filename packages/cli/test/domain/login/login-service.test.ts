...
describe("runLogin", () => {
  it("happy path: calls sessions + exchange, writes credentials, returns {apiKey, keyPrefix, label}", async () => {
    ...
    // Browser opens the sign-up-first API flow and redirects back to CLI authorization.
  ...
  it("browser open fails: prints authorizeUrl, login still completes", async () => {
    ...
    expect(h.stdout.text).toContain("https://app.midlyr.com/cli-auth?session=sess_abc&flow=cli");
  });
